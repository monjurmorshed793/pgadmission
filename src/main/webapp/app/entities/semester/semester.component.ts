...
export class SemesterComponent implements OnInit, OnDestroy {
  ...
  loadAll(): void {
    this.semesterService
      ...
      .subscribe((res: HttpResponse<ISemester[]>) => this.paginateSemesters(res.body, res.headers));
  }
  ...
  registerChangeInSemesters(): void {
    this.eventSubscriber = this.eventManager.subscribe('semesterListModification', () => this.reset());
  }
  ...
  protected paginateSemesters(data: ISemester[] | null, headers: HttpHeaders): void {
    const headersLink = headers.get('link');
    this.links = this.parseLinks.parse(headersLink ? headersLink : '');
    if (data) {
      for (let i = 0; i < data.length; i++) {
        this.semesters.push(data[i]);
      }
    }
  }
}
